...
@onready var drop_point_detector: Area2D = $droppoindetector
...
func _ready() -> void:
	Events.card_aim_started.connect(_on_card_drag_started)
	Events.card_drag_started.connect(_on_card_drag_started)
	Events.card_drag_ended.connect(_on_card_drag_ended)
	Events.card_aim_ended.connect(_on_card_drag_ended)
	card_state_machine.init(self)
	
func _input(event: InputEvent) -> void:
	card_state_machine.on_input(event)
	
func animate_to_postion(new_position: Vector2, duration: float) -> void:
